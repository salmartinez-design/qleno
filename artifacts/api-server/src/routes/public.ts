...
export async function runCalculate(params: {
  ...
  // [pricing-unify 2026-07-20] Delegate to the single shared pricing engine
  // (lib/pricing-engine.ts). The website passes ONLY its safe subset — no
  // office-only levers — so its numbers stay byte-for-byte identical to before
  // this extraction. Office-only levers live behind /api/pricing/calculate.
  const { computePricing } = await import("../lib/pricing-engine.js");
  return computePricing(params);
}
