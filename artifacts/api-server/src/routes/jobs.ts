...
// Shared cascade: drops/NULLs every child row that references jobs.id with
// ON DELETE NO ACTION, then deletes the job itself. Tables with ON DELETE
// CASCADE (job_audit_log, job_rate_mods, job_technicians) fall away with the
// parent row.
//
// Used by both force=true (owner/admin escape hatch on any non-in_progress
// status) and the non-force cancelled-job path (data-entry-mistake cleanup
// for any caller with auth).
async function cascadeDeleteJob(jobId: number, companyId: number | null) {
  await db.transaction(async (tx) => {
...
router.delete("/:id", requireAuth, async (req, res) => {
  try {
    ...
    // in_progress jobs have a tech actively on-site (clock-in in flight);
    // deleting under them produces UI ghosts and confused techs.
    ...
    // Force-delete branch: owner/admin can wipe a completed job (with its
    // timeclock entries, invoices, etc.). Cancelled jobs route through the
    // same cascade unconditionally — they are by definition discarded, so
    // any authed user can clean up data-entry mistakes without `?force`.
    if (force) {
      ...
      await cascadeDeleteJob(jobId, companyId);
    ...
    if (existing.status === "cancelled") {
      await cascadeDeleteJob(jobId, companyId);
      logAudit(req, "DELETE", "job", jobId, null, { cascade: "cancelled" });
      return res.json({ success: true, message: "Cancelled job deleted" });
    }
