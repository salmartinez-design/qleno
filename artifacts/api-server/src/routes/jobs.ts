...
// expose .execute).
type JobAddOnInput = { pricing_addon_id?: number; add_on_id?: number; qty?: number; unit_price?: number; subtotal?: number; name?: string };
export async function persistJobAddOns(exec: any, jobId: number, companyId: number, addOns: JobAddOnInput[]): Promise<void> {
  ...
  for (const a of addOns) {
    ...
    // [custom-adjustment-line 2026-07-10] Office added a free-text line from the
    // dispatch pricing editor (Maribel: "when we click edit hours and add-ons we
    // should be able to add any adjustment — parking fee, discounts, etc.").
    // There's no pricing_addon catalog row for these, so resolve/create an
    // add_ons row by the typed NAME (same case-insensitive lookup as the catalog
    // path). A negative subtotal is a discount; positive is a fee/add-on.
    const customName = String(a.name ?? "").trim();
    if (!realAddOnId && customName) {
      const existing = await exec.execute(sql`
        SELECT id FROM add_ons WHERE company_id = ${companyId} AND LOWER(name) = LOWER(${customName}) LIMIT 1
      `);
      if (existing.rows.length) {
        realAddOnId = Number((existing.rows[0] as any).id);
      } else {
        const created = await exec.execute(sql`
          INSERT INTO add_ons (company_id, name, price, category, is_active)
          VALUES (${companyId}, ${customName}, ${unitPrice}, 'other', true)
          RETURNING id
        `);
        realAddOnId = Number((created.rows[0] as any).id);
      }
  ...
  try {
    ...
    const [dup] = await db
      ...
      .values({
        ...
        flagged: false,
        // [job-created-audit 2026-08-08] Must be overridden, not inherited via
        // ...rest — the copy was booked by whoever clicked Duplicate, today,
        // not by whoever created the visit it was copied from.
        created_by_user_id: req.auth!.userId ?? null,
        created_source: "duplicate",
      } as any)
      .returning();
...
router.post("/:id/redo", requireAuth, requireRole("owner", "admin", "office"), async (req, res) => {
  try {
    ...
    const [redo] = await db.insert(jobsTable).values({
      ...
      // [job-created-audit 2026-08-08] Override, don't inherit via ...rest.
      created_by_user_id: req.auth!.userId ?? null, created_source: "redo",
    } as any).returning();
  ...
  try {
    ...
    const newJob = await db
      ...
      .values({
        ...
        // [job-created-audit 2026-08-08] Who booked it, from where. The activity
        // feed derives WHEN from created_at; these carry the WHO/HOW.
        created_by_user_id: req.auth!.userId ?? null,
        created_source: "office",
      } as any)
      .returning();
...
router.put("/:id", requireAuth, async (req, res) => {
  try {
    ...
    // [dragdrop-audit 2026-08-08] Drag-and-drop on the dispatch board is a real
    // reschedule, but this route only ever wrote an app_audit_log UPDATE row —
    // which the client/account Activity feeds don't read. So a visit moved by
    // dragging its chip changed date with no trace in the customer's history
    // (job_audit_log had 10 scheduled_date rows total, all from the modal).
    // Write the same per-field rows PATCH writes so the feed reads the same for
    // both paths. Non-fatal — an audit failure must never fail the move.
    try {
      const auditFields: Array<{ field: string; old: any; next: any }> = [];
      if (scheduled_date !== undefined && priorSched && String(priorSched.scheduled_date ?? "").slice(0, 10) !== String(scheduled_date).slice(0, 10)) {
        auditFields.push({ field: "scheduled_date", old: String(priorSched.scheduled_date ?? "").slice(0, 10) || null, next: String(scheduled_date).slice(0, 10) });
      }
      if (scheduled_time !== undefined && priorSched && (priorSched.scheduled_time ?? null) !== (scheduled_time ?? null)) {
        auditFields.push({ field: "scheduled_time", old: priorSched.scheduled_time ?? null, next: scheduled_time ?? null });
      }
      if (auditFields.length) {
        const companyId = req.auth!.companyId!;
        const userId = req.auth!.userId ?? null;
        const ur = await db.execute(sql`SELECT first_name, last_name, email FROM users WHERE id = ${userId} LIMIT 1`);
        ...
        for (const f of auditFields) {
          await db.execute(sql`
            ...
            VALUES
              (${jobId}, ${companyId}, ${userId}, ${actorName}, ${String(u.email ?? "")},
               ${f.field},
               ${JSON.stringify(f.old)}::jsonb,
               ${JSON.stringify(f.next)}::jsonb,
               'this_job', ${null})`);
        }
      }
    } catch (auditErr) {
      console.error("[PUT /jobs/:id] reschedule audit non-fatal:", auditErr);
    }
              ...
              const insertRes = await tx.execute(sql`
                INSERT INTO jobs
                  (company_id, client_id, assigned_user_id, service_type, status,
                   ...
                   -- [job-created-audit 2026-08-08] Generated by the recurring
                   -- engine, not booked by a person — creator stays NULL and the
                   -- activity feed says so rather than guessing.
                   created_source)
                VALUES
                  (${r.company_id}, ${r.client_id}, ${primaryUid},
                   ...
                   ${r.booking_location}, ${r.address_zip}, 'recurring')
                RETURNING id
              `);
