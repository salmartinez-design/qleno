...
import { type ClockEntry } from "./payroll-compute.js";
...
export async function computePeriodPay(companyId: number, start: string, end: string): Promise<PeriodPay[]> {
  const co = (await db.execute(sql`SELECT res_tech_pay_pct, deep_clean_pay_pct, move_in_out_pay_pct, commercial_hourly_rate, commercial_comp_mode FROM companies WHERE id = ${companyId} LIMIT 1`)).rows[0] as any;
  const resRates = parseResRatesRow(co);
  const commercialHourlyRate = parseFloat(String(co?.commercial_hourly_rate ?? 20));
  const commercialCompMode: "allowed_hours" | "actual_hours" = co?.commercial_comp_mode === "actual_hours" ? "actual_hours" : "allowed_hours";
  const jr = (await db.execute(sql`
    SELECT j.id, j.assigned_user_id, j.account_id, cl.client_type, cl.first_name, cl.last_name, a.account_name, j.service_type,
  ...
  const nameOfJob = new Map<number, string>(jr.map(j => [Number(j.id), (`${j.first_name ?? ""} ${j.last_name ?? ""}`.trim() || j.account_name || "")]));
  const ids = jr.map(j => Number(j.id));
  if (!ids.length) return [];

  // ── Same inputs, same engine as GET /payroll/detail (Option A parity). ──────
  // source='punched' MATCHES the period-lock paycheck path so the published
  // snapshot reads the SAME hours the office screen shows — estimated/imported
  // clocks never split pay.
  const clocks: ClockEntry[] = ((await db.execute(sql`
    ...
    FROM timeclock WHERE company_id = ${companyId} AND job_id = ANY(ARRAY[${sql.raw(intList(ids))}]::int[]) AND clock_out_at IS NOT NULL AND source = 'punched'
  ...
  if (clockedUserIds.length) {
    const us = (await db.execute(sql`SELECT id, first_name, last_name, is_sandbox FROM users WHERE company_id = ${companyId} AND id = ANY(ARRAY[${sql.raw(intList(clockedUserIds))}]::int[])`)).rows as any[];
    for (const u of us) {
  ...
  // per-job final_pay DOLLAR overrides (job_technicians) still win
  ...
  // per-(tech, job) clocked hours map ("job:user" → hours) — note key ORDER
  // (job first) differs from the overrides maps (user first); mirrors /detail.
  ...
  // THIS is what carries the Time Clock "Save pay" edits (fee-split / hourly /
  // trainee / breakage) into the published snapshot — the whole point of the fix.
  ...
  try {
    const tr = await db.execute(sql`
      SELECT job_id, user_id, is_primary, pay_type, hourly_rate, commission_pct,
       ...
       WHERE company_id = ${companyId} AND job_id = ANY(ARRAY[${sql.raw(intList(ids))}]::int[])`);
  ...
  for (const l of lines) {
    ...
    p.hours = r2(p.hours + (l.paidHours > 0 ? l.paidHours : l.clockedHours));
    p.jobs.push({ job_id: l.job_id, date: l.scheduled_date, client: nameOfJob.get(l.job_id) || "", amount: l.amount, hours: l.paidHours > 0 ? l.paidHours : l.clockedHours, basis: l.payBasisLabel });
  }
  // fold additional_pay
