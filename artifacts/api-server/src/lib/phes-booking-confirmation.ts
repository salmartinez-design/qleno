...
// T&C). Only booking-specific values (name, date, window, address, service, the
// itemized breakdown, links, contact) are interpolated.
//
...
export interface PhesConfOpts {
  ...
  link: string | null;    // appointment view link
}

// A brand-blue H3 section heading with a hairline underline.
...
// A label/value row for the details card.
...
// A colored callout table with a small circular icon badge.
...
export function renderPhesBookingConfirmation(o: PhesConfOpts): string {
  ...
  const cta = o.link ? `
    <table role="presentation" cellpadding="0" cellspacing="0" align="center" style="margin:24px auto 8px;"><tr>
      <td bgcolor="${BRAND}" align="center" style="border-radius:8px;">
        <a href="${escAttr(o.link)}" style="display:inline-block;padding:13px 28px;font-family:${FONT};font-size:15px;font-weight:700;color:#ffffff;text-decoration:none;border-radius:8px;">View your appointment</a>
      </td>
    </tr></table>` : "";
...
<table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="background:${BG};padding:24px 14px;"><tr><td align="center">
  <table role="presentation" width="600" cellpadding="0" cellspacing="0" style="max-width:600px;width:100%;background:#ffffff;border:1px solid ${BORDER};border-radius:14px;overflow:hidden;">
    ...
    <tr><td style="padding:26px 32px 30px;">
      <p style="margin:0 0 20px;font-family:${FONT};font-size:15px;color:${INK};line-height:1.6;">Hi ${esc(o.firstName) || "there"}, thanks for booking with ${esc(o.companyName)}. Here are your details:</p>
      ...
      <ul style="margin:0 0 8px;padding-left:20px;font-family:${FONT};font-size:14px;color:${INK};line-height:1.9;">
        ...
        <li>One free reschedule per appointment. Additional reschedules count as a late cancel.</li>
      ...
      <!-- Fine print -->
      <p style="margin:22px 0 0;font-family:${FONT};font-size:12px;color:#9E9B94;line-height:1.6;"><strong style="color:${MUTE};">Pricing:</strong> Flat-rate estimates assume the home matches what you described. If conditions differ significantly, we'll send an updated estimate. Extra time bills at $65/hour per cleaner.</p>
