...
const SAMPLE: Record<string, string> = {
  ...
  company_phone: "(708) 555-0123", company_email: "info@phes.io", service_type: "Standard Cleaning",
  date: "Friday, June 27, 2026", appointment_date: "Friday, June 27, 2026",
  time: "9:00 AM CT", appointment_time: "9:00 AM CT",
  ...
  appointment_link: "https://app.qleno.com/appt/sample", review_link: "https://app.qleno.com/review/sample",
...
export function EasyMessageEditor({
  ...
  templateId, saving, onSave, onCancel,
}: {
  channel: "email" | "sms";
  ...
  // baseline so the editor can show "Sending DRAFT" (unsaved edits present) vs
  // "Sending SAVED". The test always sends the CURRENT editor content.
  ...
  async function sendTest() {
    if (!templateId || !ref.current) return;
    setTesting(true); setTestMsg(null);
    try {
      const r = await fetch(`${API}/api/notifications/templates/${templateId}/test`, {
        method: "POST",
        headers: { ...getAuthHeaders(), "Content-Type": "application/json" },
        body: JSON.stringify({ subject, body: editableToBody(ref.current, channel), channel }),
      });
