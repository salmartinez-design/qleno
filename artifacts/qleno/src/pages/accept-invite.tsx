  ...
  return (
    <div style={{ display:'flex', alignItems:'center', gap:8 }}>
      <div style={{ width:16, height:16, borderRadius:8, display:'flex', alignItems:'center', justifyContent:'center',
        background: met ? '#E6F6F1' : '#F0EEE9', border: `1px solid ${met?'#C7E7DE':'#E5E2DC'}` }}>
        {met && <Check size={10} color="#0F7A63" strokeWidth={3}/>}
      </div>
      <span style={{ fontSize:12, color: met ? '#0F7A63' : '#6B6860' }}>{label}</span>
    </div>
  );
}
    ...
    return (
      <div style={{ minHeight:'100vh', background:'#F7F6F3', display:'flex', alignItems:'center', justifyContent:'center', padding:20 }}>
        <div style={{ background:'#FFFFFF', borderRadius:12, padding:'40px 36px', width:'100%', maxWidth:440, boxShadow:'0 4px 24px rgba(0,0,0,0.08)', textAlign:'center' }}>
          <div style={{ width:48,height:48,borderRadius:24,background:'#FCEBEA',display:'flex',alignItems:'center',justifyContent:'center',margin:'0 auto 16px' }}>
            <span style={{ fontSize:14, fontWeight:700, color:'#B3261E' }}>X</span>
          </div>
          <h2 style={{ fontSize:20, fontWeight:700, color:'#1A1917', margin:'0 0 8px 0' }}>Invalid Invite</h2>
          <p style={{ fontSize:14, color:'#6B6860', margin:'0 0 20px 0' }}>{invalid}</p>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'#F7F6F3', display:'flex', alignItems:'center', justifyContent:'center', padding:20, fontFamily:"'Plus Jakarta Sans', sans-serif" }}>
      <div style={{ background:'#FFFFFF', borderRadius:12, padding:'40px 36px', width:'100%', maxWidth:440, boxShadow:'0 4px 24px rgba(0,0,0,0.08)' }}>
        ...
        {done && (
          <div style={{ background:'#E6F6F1', border:'1px solid #C7E7DE', borderRadius:8, padding:'12px 16px', marginBottom:16, display:'flex', alignItems:'center', gap:8 }}>
            <Check size={16} color="#0F7A63"/>
            <p style={{ fontSize:13, color:'#0F7A63', margin:0, fontWeight:600 }}>Account created! Redirecting…</p>
          </div>
        )}
        ...
        <form onSubmit={handleSubmit}>
          ...
          <div style={{ marginBottom:20 }}>
            ...
            <input type={showPw ? 'text' : 'password'} value={confirm} onChange={e => setConfirm(e.target.value)}
              ...
              style={{ width:'100%', height:38, padding:'0 12px', border:`1px solid ${confirm && confirm!==password?'#B3261E':'#E5E2DC'}`, borderRadius:8, fontSize:13, color:'#1A1917', background:'#FFFFFF', outline:'none' }}/>
          </div>
