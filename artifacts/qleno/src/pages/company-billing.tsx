...
const STATUS_CONFIG: Record<string, { label: string; bg: string; text: string; icon: any }> = {
  trialing:  { label: "Trial", bg: "#FDF3E4", text: "#B45309", icon: Clock },
  active:    { label: "Active", bg: "#D1FAE5", text: "#065F46", icon: CheckCircle },
  past_due:  { label: "Past Due", bg: "#FCEBEA", text: "#B3261E", icon: AlertCircle },
  canceled:  { label: "Canceled", bg: "#F0EEE9", text: "#6B6860", icon: X },
};
    ...
    <DashboardLayout>
      <div style={{ padding: "28px 32px", maxWidth: "900px", margin: "0 auto" }}>
        <div style={{ marginBottom: "28px" }}>
          ...
          <div style={{ fontSize: "13px", color: "#6B6860", marginTop: "4px" }}>Manage your Qleno subscription</div>
        </div>
          ...
          <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", marginBottom: "20px" }}>
            <div>
              ...
              <div style={{ fontSize: "28px", fontWeight: 800, color: "var(--brand)" }}>
                ${billing?.monthly_total || 100}<span style={{ fontSize: "14px", fontWeight: 500, color: "#6B6860" }}>/month</span>
              </div>
            </div>
        ...
        <div style={{ ...card, marginBottom: "20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "8px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #F0EEE9" }}>
              <div>
                <div style={{ fontSize: "13px", fontWeight: 600, color: "#1A1917" }}>Base Plan ({billing?.plan_label || "Team"})</div>
                <div style={{ fontSize: "12px", color: "#6B6860" }}>Qleno starts at $100/mo for solo operators, $200/mo for teams up to 10, and $250/mo for up to 20 technicians</div>
              </div>
            ...
            {(billing?.employee_count || 0) > 20 && (
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "10px 0", borderBottom: "1px solid #F0EEE9" }}>
                <div>
                  ...
                  <div style={{ fontSize: "12px", color: "#6B6860" }}>{(billing.employee_count - 20)} over 20 × $5.00/mo</div>
                </div>
        ...
        {status === "trialing" && (
          <div style={{ ...card, borderLeft: "3px solid #F59E0B", marginBottom: "20px", display: "flex", alignItems: "flex-start", gap: "12px" }}>
            ...
            <div>
              ...
              <div style={{ fontSize: "13px", color: "#6B6860" }}>Your trial gives you full access to all features. Add a payment method before your trial ends to continue uninterrupted service.</div>
            </div>
          </div>
        )}
        ...
        {tierSlug === "solo" && (
          <div style={{ ...card, borderLeft: "3px solid #6B6860", marginBottom: "20px", display: "flex", alignItems: "flex-start", gap: "14px" }}>
            ...
            <div style={{ flex: 1 }}>
              ...
              <div style={{ fontSize: "13px", color: "#6B6860", marginBottom: "12px" }}>The Solo plan is designed for owner-operators with 1 user. Upgrade to Team or Pro to unlock employee management, payroll, timeclock, and dispatch board features.</div>
              <button
        ...
        {/* Danger zone */}
        <div style={{ ...card, borderColor: "#F1D0CB" }}>
          <div style={{ fontSize: "14px", fontWeight: 700, color: "#B3261E", marginBottom: "8px" }}>Cancel Subscription</div>
          <div style={{ fontSize: "13px", color: "#6B6860", marginBottom: "16px" }}>
          ...
          {showCancel ? (
            <div style={{ backgroundColor: "#FCEBEA", border: "1px solid #F1D0CB", borderRadius: "8px", padding: "16px" }}>
              <div style={{ fontSize: "13px", fontWeight: 700, color: "#B3261E", marginBottom: "8px" }}>Are you sure you want to cancel?</div>
              <div style={{ display: "flex", gap: "8px" }}>
                <button onClick={() => setShowCancel(false)} style={{ backgroundColor: "#F0EEE9", color: "#6B6860", border: "none", borderRadius: "6px", padding: "8px 14px", fontSize: "13px", cursor: "pointer" }}>Keep Subscription</button>
                <button onClick={() => cancelMut.mutate()} disabled={cancelMut.isPending} style={{ backgroundColor: "#B3261E", color: "#FFFFFF", border: "none", borderRadius: "6px", padding: "8px 14px", fontSize: "13px", fontWeight: 700, cursor: "pointer" }}>
                  {cancelMut.isPending ? "Cancelling..." : "Yes, Cancel Subscription"}
                </button>
              </div>
            </div>
          ) : (
            <button onClick={() => setShowCancel(true)} style={{ backgroundColor: "transparent", color: "#B3261E", border: "1px solid #F1D0CB", borderRadius: "6px", padding: "8px 14px", fontSize: "13px", fontWeight: 600, cursor: "pointer" }}>
              Cancel Subscription
            </button>
          )}
        </div>
          ...
          <div style={{ position: "fixed", inset: 0, backgroundColor: "rgba(0,0,0,0.5)", zIndex: 1000, display: "flex", alignItems: "center", justifyContent: "center", padding: "24px" }}>
            <div style={{ backgroundColor: "#FFFFFF", borderRadius: "16px", padding: "32px", maxWidth: "640px", width: "100%" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: "16px", marginBottom: "24px" }}>
                {plans.map(p => (
                  <div key={p.id} onClick={() => setSelectedPlan(p.id)} style={{ border: `2px solid ${selectedPlan === p.id ? "var(--brand)" : "#E5E2DC"}`, borderRadius: "10px", padding: "20px", cursor: "pointer", backgroundColor: selectedPlan === p.id ? "#EFEFF2" : "#FFFFFF", transition: "all 0.15s" }}>
                    ...
                    <div style={{ fontSize: "12px", color: "#6B6860", marginBottom: "12px" }}>{p.desc}</div>
                    <div style={{ display: "flex", flexDirection: "column", gap: "6px" }}>
                      {p.features.map(f => (
                        <div key={f} style={{ display: "flex", alignItems: "center", gap: "6px", fontSize: "12px", color: "#1A1917" }}>
                          <CheckCircle size={12} style={{ color: "#0F7A63", flexShrink: 0 }} /> {f}
                        </div>
                      ))}
                    </div>
                  </div>
              ...
              <div style={{ backgroundColor: "#F7F6F3", borderRadius: "8px", padding: "12px 16px", marginBottom: "16px", fontSize: "12px", color: "#6B6860" }}>
