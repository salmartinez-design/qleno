...
export default function AdminBilling() {
  ...
  if (loading) {
    return <AdminLayout title="Billing & Revenue"><div style={{ color: "#6B6860", textAlign: "center", paddingTop: "60px" }}>Loading...</div></AdminLayout>;
  }
  if (!data) {
    return <AdminLayout title="Billing & Revenue"><div style={{ color: "#B3261E", textAlign: "center", paddingTop: "60px" }}>Failed to load billing data.</div></AdminLayout>;
  }
    ...
    <AdminLayout title="Billing & Revenue">
      <div style={{ display: "flex", flexDirection: "column", gap: "24px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: "12px" }}>
          ...
          <StatCard label="Platform Fees (5%)" value={`$${data.platformFees.toLocaleString()}`} sub="This month" color="#0F7A63" />
          <StatCard label="Upcoming Renewals" value={String(data.upcomingRenewals)} color="#2F3646" />
          <StatCard label="Failed Payments" value={String(data.failedPayments)} color="#B3261E" />
        </div>
            ...
            {data.mrrHistory.map(m => {
              ...
              return (
                <div key={m.month} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: "6px", height: "100%", justifyContent: "flex-end" }}>
                  <span style={{ fontSize: "11px", color: "#6B6860", fontWeight: 500 }}>${(m.mrr / 1000).toFixed(1)}k</span>
                  <div style={{
                    width: "100%",
        ...
        <div style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E2DC", borderRadius: "10px", padding: "20px" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "14px" }}>
            {planKeys.map(plan => {
              ...
              const colors: Record<string, string> = { starter: "#9E9B94", growth: PURPLE, enterprise: "#B45309" };
              return (
                <div key={plan}>
        ...
        {/* Stripe note */}
        <div style={{
          backgroundColor: "#EFEFF2", border: "1px solid #DEDEE4",
          borderRadius: "10px", padding: "16px 20px",
          display: "flex", alignItems: "flex-start", gap: "10px",
        }}>
          <span style={{ fontSize: "16px" }}>ℹ</span>
          <div>
            <p style={{ fontSize: "13px", fontWeight: 600, color: "#2F3646", margin: "0 0 4px" }}>Stripe Integration Pending</p>
            <p style={{ fontSize: "12px", color: "#6B6860", margin: 0 }}>Revenue figures shown are derived from active subscription plans. Connect Stripe to see real payment data, failed charges, and upcoming renewals.</p>
          </div>
        </div>
      </div>
    </AdminLayout>
