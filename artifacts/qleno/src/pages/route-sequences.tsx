      ...
      <div style={{ fontFamily: FF, maxWidth: 900, margin: "0 auto" }}>
        ...
        {isLoading ? (
          ...
          <div style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E2DC", borderRadius: 12, padding: 48, textAlign: "center" }}>
            ...
            <p style={{ margin: "0 0 20px", fontSize: 13, color: "#6B6860" }}>Create a route to sequence jobs and assign crews for the day</p>
            <button
              onClick={() => setShowCreate(true)}
              ...
              <div key={route.id} style={{ backgroundColor: "#FFFFFF", border: "1px solid #E5E2DC", borderRadius: 10, padding: "18px 20px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                  <div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
                    <div style={{ marginTop: 2, color: "#E5E2DC", cursor: "grab" }}>
                      <GripVertical size={16} />
                    </div>
                    <div>
                      <div style={{ display: "flex", gap: 8, alignItems: "center", marginBottom: 4 }}>
                        <h3 style={{ margin: 0, fontSize: 14, fontWeight: 700, color: "#1A1917" }}>{route.name}</h3>
                        <span style={{ fontSize: 11, color: "#9E9B94", backgroundColor: "#F0EEE9", padding: "2px 7px", borderRadius: 4 }}>
                        ...
                        {route.status && (
                          <span style={{
                            ...
                            backgroundColor: route.status === "active" ? "#E6F6F1" : "#F0EEE9",
                            color: route.status === "active" ? "#0F7A63" : "#6B6860",
                          }}>{route.status}</span>
                        )}
                      </div>
                      {route.notes && <p style={{ margin: 0, fontSize: 12, color: "#6B6860" }}>{route.notes}</p>}
