...
function InlinePricingEditor({ job, canEdit, onUpdate, adjustments, modLines, tipsTotal = 0 }: { job: DispatchJob; canEdit: boolean; onUpdate: () => void; adjustments?: React.ReactNode; modLines?: Array<{ id: number; mod_type: "time" | "flat"; minutes: number | null; amount: string; reason: string }>; tipsTotal?: number }) {
  ...
  // [flat-addon-itemize 2026-07-11] Derive the service line from base_fee (the
  // all-in service+add-ons amount BEFORE adjustments), NOT from `amount`.
  // `amount` folds in rate-mods (the discount/additional-time adjustments), so
  // basing the service line on it let an adjustment inflate "Deep Clean" — the
  // exact thing the office flagged (Joni's $62.40 window + $72 of mods bleeding
  // into the base line). base_fee − add-ons is the pure service, and the mods
  // render in their own Adjustments section, so the base line can never move.
  // Falls back to `amount` only when base_fee isn't on the payload (older cache).
  const baseSource = job.base_fee != null ? Number(job.base_fee) : total;
  const baseInit = Math.max(0, Math.round((baseSource - addOnSum) * 100) / 100);
    ...
    <>
      ...
      <div style={panelStyle}>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "16px 20px" }}>
          ...
          {/* [omw-on-the-job 2026-08-20] Did the cleaner tell the customer they
              were coming, and did the text actually land? The tap has been
              recorded since cutover 1C but no office screen ever showed it, so
              the only way to answer was to ask the tech. That blind spot is how
              #1535 stayed hidden for three months: 72 taps, not one customer
              texted, and a "customer not told" flag that looked identical to a
              customer who had opted out. This separates the two — and says
              plainly when nobody tapped at all. */}
          {(() => {
            const omw = job.on_my_way;
            // Nothing to answer for a job that hasn't happened yet, or one that
            // was called off. Everything from today backwards gets the section
            // even with no tap, because "nobody told the customer" IS the answer.
            const todayStr = new Date().toLocaleDateString("en-CA", { timeZone: "America/Chicago" });
            const isPastOrToday = !!job.scheduled_date && job.scheduled_date <= todayStr;
            if (!omw && (!isPastOrToday || job.status === "cancelled")) return null;

            // Plain English, no status codes. The office reads this to decide
            // whether to pick up the phone, so every line has to say what it
            // means without a lookup table.
            const told: { text: string; color?: string; detail?: string } = (() => {
              if (!omw) return { text: "No one tapped On my way", color: "#B45309" };
              if (omw.client_notified) return { text: "Yes, text delivered", color: "#0F7A63" };
              if (omw.deferred) return { text: "No, the cleaner chose to wait" };
              switch (omw.sms_result) {
                case "suppressed_client_opted_out":
                  return { text: "No, this customer is set to get no texts" };
                case "suppressed_no_phone":
                  return { text: "No, no phone number on file", color: "#B45309" };
                case "suppressed_tenant_disabled":
                  return { text: "No, on-my-way texts are turned off", color: "#B45309" };
                case "suppressed_comms_disabled":
                  return { text: "No, all texts are turned off", color: "#B45309" };
                case "error":
                  return { text: "No, the text failed to send", color: "#B3261E", detail: omw.sms_error || undefined };
                default:
                  // Every tap before #1535 stored the outcome as a bare boolean,
                  // so there is no reason on file for those rows. Say so rather
                  // than guessing one.
                  return { text: "No, reason not recorded", color: "#B45309" };
              }
            })();
            const bad = !omw || (!omw.client_notified && !omw.deferred);
            return (
              <PS
                label="On My Way"
                summary={omw
                  ? `${fmtClock(omw.tapped_at)} · ${omw.client_notified ? "customer texted" : "customer not texted"}`
                  : "never tapped"}
                defaultOpen={bad}
              >
                {omw ? (
                  <>
                    <KV label="Tapped" value={`${fmtClock(omw.tapped_at)}${omw.by_name ? ` by ${omw.by_name}` : ""}`} />
                    {omw.promised_arrival_at && (
                      <KV label="Told customer" value={fmtClock(omw.promised_arrival_at)} />
                    )}
                    <KV label="Customer texted" value={told.text} color={told.color} />
                    {told.detail && <KV label="What went wrong" value={told.detail} color="#B3261E" />}
                    {omw.eta_past_start && (
                      <KV label="Heads up" value="The promised time was after the scheduled start" color="#B45309" />
                    )}
                  </>
                ) : (
                  <KV label="Customer texted" value={told.text} color={told.color} />
...
function MobileCalendarView({ jobs, onJobClick, isToday }: {
  ...
  // ~8 jobs fit per screen instead of ~3 (Sal request). Zone fill + every
  // status treatment (completed check, active ring, no-show badge, cancel
  // desaturate/strikethrough) carry over unchanged.
  const Row = ({ j }: { j: DispatchJob }) => {
    ...
    return (
      <button onClick={() => onJobClick(j)} className={visual.glowActive ? "qleno-active-glow" : undefined} style={{
        minWidth: 0, backgroundColor: color, borderRadius: 13, border: "none", textAlign: "left",
        padding: "9px 11px 10px", cursor: "pointer", fontFamily: FF, boxSizing: "border-box",
        ...
        <div style={{ fontSize: 9.5, fontWeight: 700, color: sub, opacity: 0.9 }}>ends {fmtRail(eMin)}</div>
      </button>
    );
  };
...
function JobHoverCard({ job, assignedName }: { job: DispatchJob; assignedName?: string }) {
  ...
  return (
    ...
    <div ref={popoverRef} style={{
      ...
      {/* [batch-tip-mislabel 2026-08-13] Two corrections to the block above.
          (1) The tip is READ, not inferred. It was `invoice_total − amount`,
          which silently absorbs every other line on the invoice. (2) A
          COMBINED invoice's total is never this job's total — it covers the
          other visits folded onto the same document, so leading with it
          overstates one visit by the rest of the batch. Heritage Condominium
          hit both at once: a $108.25 weekly visit on a $433.00 combined
          invoice, rendered as "$433.00 — Service $108.25 · Tip $324.75". */}
      {(() => {
        const svcAmount = job.amount || 0;
        const invTotal = job.invoice_total != null ? Number(job.invoice_total) : null;
        const tip = job.invoice_tips != null ? Number(job.invoice_tips) : 0;
        const isBatch = !!job.invoice_is_batch;
        // Lead with the invoice total only when the invoice covers THIS job
        // alone and actually exceeds the service amount.
        const showInvoice = !isBatch && invTotal != null && invTotal - svcAmount > 0.005;
        const otherCharges = showInvoice ? invTotal! - svcAmount - tip : 0;
        return (
        ...
        <div>
          ...
          {showInvoice && (
            <div style={{ fontSize: 12, color: "#6B6860", marginTop: 2 }}>
              Service ${svcAmount.toFixed(2)}
              {tip > 0.005 && ` · Tip $${tip.toFixed(2)}`}
              {/* Anything left over is an invoice-level adjustment. Name it
                  that rather than folding it into the tip. */}
              {otherCharges > 0.005 && ` · Adjustments $${otherCharges.toFixed(2)}`}
            </div>
          )}
          {/* On a combined invoice the total belongs to the document, not this
              visit — show what this visit contributed and point at the invoice
              it was billed on. */}
          {isBatch && invTotal != null && (
            <div style={{ fontSize: 12, color: "#6B6860", marginTop: 2 }}>
              This visit{job.invoice_number ? ` · on combined invoice #${job.invoice_number}` : " · on a combined invoice"} (${invTotal.toFixed(2)} total)
            </div>
          )}
        </div>
        {paymentLabel && (
...
// [2026-07-15] Events (incl. 1-on-1) render as real bars — a bit shorter than
      ...
      <DndContext sensors={sensors} onDragStart={onDragStart} onDragEnd={onDragEnd}>
        <div style={{ display: "flex", height: "calc(100vh - 56px)", overflow: "hidden", fontFamily: FF, flexDirection: "column" }}>
          ...
          <div style={{ padding: "8px 16px", borderBottom: "1px solid #E5E2DC", backgroundColor: "#FFFFFF", display: "flex", alignItems: "center", gap: 10, flexShrink: 0, flexWrap: "nowrap" }}>
            {/* Date nav */}
            <div style={{ display: "flex", alignItems: "center", gap: 5 }}>
              {/* [today-btn-left 2026-07-15] Today lives on the FAR LEFT (Google-
                  Calendar style) so the › next arrow is always the rightmost nav
                  control — hitting next never lands you on a Today button that
                  bounces you back to today. Fixed-width slot so the arrows never
