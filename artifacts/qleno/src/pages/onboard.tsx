...
function SignaturePad({ onSignature }: { onSignature: (data: string, name: string) => void }) {
  ...
  useEffect(() => {
    if (mode === "type" && typedName) {
      onSignature("typed:" + typedName, typedName);
    }
  }, [typedName, mode]);
        ...
        <div style={{ position: "relative" }}>
          ...
          {hasDrawn && (
            <button onClick={clear} style={{
              ...
              fontSize: 11, color: "#9E9B94", background: "#F7F6F3",
              border: "1px solid #E5E2DC", borderRadius: 4, padding: "2px 8px", cursor: "pointer",
            }}>Clear</button>
          )}
        </div>
      ) : (
...
function DocView({
  ...
  const needsScroll = docContent.length > 800;

  useEffect(() => {
    if (!needsScroll) { setScrolled(true); return; }
  ...
  const canProceed = scrolled && checked && (!doc.requires_signature || (signerName.trim() && (signature || !doc.requires_signature)));

  const handleSubmit = async () => {
    setSubmitting(true);
    setError("");
    try {
      const r = await fetch(`${API}/api/document-requests/onboard/${token}/sign`, {
        method: "POST",
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{
        background: "#F7F6F3", borderRadius: 8, padding: "10px 16px",
      ...
      <h2 style={{ fontSize: 20, fontWeight: 700, color: "#1A1917", margin: 0 }}>{doc.template_name}</h2>
      ...
      {needsScroll && !scrolled && (
        <p style={{ fontSize: 12, color: "#9E9B94", textAlign: "center", margin: 0 }}>
          Please scroll to the bottom to continue.
        </p>
      )}

      {doc.requires_signature && scrolled && (
        <div style={{ border: "1px solid #E5E2DC", borderRadius: 10, padding: 20 }}>
          ...
          <div style={{ marginBottom: 12 }}>
            <label style={{ fontSize: 11, fontWeight: 700, color: "#9E9B94", textTransform: "uppercase", display: "block", marginBottom: 4 }}>Your Name</label>
            <input
              value={signerName}
      ...
      >
        {submitting ? "Submitting..." : doc.requires_signature ? "Sign and Continue" : "Acknowledge and Continue"}
...
export default function OnboardPage() {
  ...
  const { data, isLoading, error } = useQuery<OnboardData>({
    queryKey: ["onboard", token],
    queryFn: async () => {
      const r = await fetch(`${API}/api/document-requests/onboard/${token}`);
      if (!r.ok) {
  ...
  const handleDocComplete = (idx: number) => {
    setCompletedDocs(prev => new Set([...prev, idx]));
    if (idx + 1 >= (data?.docs.length || 0)) {
      setCompleted(true);
    } else {
      setCurrentIdx(idx + 1);
    }
  ...
  if (error || !data) {
    ...
    return (
      <div style={containerStyle}>
        <div style={{ width: 480, background: "#fff", border: "1px solid #E5E2DC", borderRadius: 16, padding: 40, textAlign: "center" }}>
  ...
  if (completed) {
    return (
      <div style={containerStyle}>
        <div style={{ width: 480, background: "#fff", border: "1px solid #E5E2DC", borderRadius: 16, padding: 48, textAlign: "center" }}>
  ...
  const currentDoc = data.docs[currentIdx];
    ...
    <div style={containerStyle}>
      <div style={{ width: "100%", maxWidth: 640, display: "flex", flexDirection: "column", gap: 20 }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          {data.company_logo && (
            <img src={data.company_logo} style={{ height: 32, objectFit: "contain" }}/>
          )}
          <span style={{ fontSize: 14, fontWeight: 700, color: "#1A1917" }}>{data.company_name}</span>
        </div>
        <div style={{ background: "#fff", border: "1px solid #E5E2DC", borderRadius: 16, padding: "28px 32px" }}>
          <DocView
            doc={currentDoc}
            index={currentIdx}
            ...
            onComplete={() => handleDocComplete(currentIdx)}
          />
        </div>
      </div>
    </div>
