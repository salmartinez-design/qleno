  ...
  return (
    <DashboardLayout>
      <div style={{ display: 'flex', flexDirection: 'column', gap: '28px' }}>
        {/* Header */}
        <div>
          <h1 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: '42px', color: '#F0EDE8', margin: 0, lineHeight: 1.1 }}>Cleancyclopedia</h1>
          <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 300, fontSize: '13px', color: '#7A7873', marginTop: '6px' }}>Training library, SOPs, chemical guides, and client communication scripts.</p>
        </div>

        {/* Search */}
        <div style={{ position: 'relative', maxWidth: '480px' }}>
          <Search size={16} strokeWidth={1.5} style={{ position: 'absolute', left: '14px', top: '50%', transform: 'translateY(-50%)', color: '#7A7873' }} />
          <input
            placeholder="Search articles, techniques, chemicals..."
            value={search}
            onChange={e => setSearch(e.target.value)}
            style={{ width: '100%', paddingLeft: '44px', paddingRight: '16px', height: '44px', backgroundColor: '#161616', border: '1px solid #252525', borderRadius: '8px', color: '#F0EDE8', fontSize: '14px', fontFamily: "'Plus Jakarta Sans', sans-serif", outline: 'none' }}
          />
        </div>
        ...
        {filtered.map(cat => (
          <div key={cat.category}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '10px', marginBottom: '12px' }}>
              ...
              <h2 style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 700, fontSize: '20px', color: '#F0EDE8', margin: 0 }}>{cat.category}</h2>
            </div>
              ...
              {cat.items.map(article => (
                ...
                >
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '16px 20px' }}>
                    <div style={{ flex: 1 }}>
                      <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 400, fontSize: '14px', color: '#F0EDE8', margin: '0 0 3px 0' }}>{article.title}</p>
                      <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 300, fontSize: '12px', color: '#7A7873', margin: 0 }}>{article.desc}</p>
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginLeft: '16px', flexShrink: 0 }}>
                      <span style={{ fontSize: '11px', fontFamily: "'Plus Jakarta Sans', sans-serif", color: '#7A7873', backgroundColor: '#0D0D0D', border: '1px solid #252525', padding: '3px 10px', borderRadius: '4px' }}>{article.time}</span>
                      <ChevronRight size={14} strokeWidth={1.5} style={{ color: '#7A7873', transform: expanded === article.title ? 'rotate(90deg)' : 'none', transition: 'transform 0.15s' }} />
                    </div>
                  </div>
                  {expanded === article.title && (
                    <div style={{ padding: '0 20px 20px', borderTop: '1px solid #252525' }}>
                      <div style={{ paddingTop: '16px' }}>
                        <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 300, fontSize: '13px', color: '#7A7873', lineHeight: 1.7, margin: 0 }}>
                        ...
                        <div style={{ display: 'flex', gap: '10px', marginTop: '16px' }}>
                          <button style={{ padding: '7px 16px', backgroundColor: 'var(--tenant-color)', color: '#0D0D0D', borderRadius: '6px', fontSize: '12px', fontFamily: "'Plus Jakarta Sans', sans-serif", border: 'none', cursor: 'pointer' }}>
                            Read Full Article
                          </button>
                          <button style={{ padding: '7px 16px', border: '1px solid #252525', borderRadius: '6px', backgroundColor: 'transparent', color: '#7A7873', fontSize: '12px', fontFamily: "'Plus Jakarta Sans', sans-serif", cursor: 'pointer' }}>
                            Download PDF
                          </button>
                        </div>
                      </div>
        ...
        {filtered.length === 0 && (
          <div style={{ textAlign: 'center', padding: '64px 0', border: '1px dashed #252525', borderRadius: '10px' }}>
            <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontSize: '18px', color: '#F0EDE8', margin: '0 0 8px 0' }}>No articles found</p>
            <p style={{ fontFamily: "'Plus Jakarta Sans', sans-serif", fontWeight: 300, fontSize: '13px', color: '#7A7873', margin: 0 }}>Try a different search term.</p>
          </div>
        )}
      </div>
    </DashboardLayout>
