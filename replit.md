...
- **API Routes:** A comprehensive set of API routes covering authentication, user management, payroll, dispatch, HR, document management, and mileage requests. New routes: `GET/PATCH /api/payroll-settings` (detailed compensation settings per company), `GET/PATCH /api/compliance-settings` (HR/legal compliance toggles with upsert), `GET /api/subscription/tiers` (public tier list), `GET /api/subscription/me` (company tier + seat counts + overage calc), `GET /api/subscription/feature-gates` (per-company feature flags), `GET /api/admin/tenants` (super admin: all companies with tier, user counts, MRR).
- **Multi-Tenant SaaS Tables (raw SQL, not in Drizzle schema):** `subscription_tiers` (Solo $100/mo 0 techs, Team $200/mo 10 techs, Pro $250/mo 50 techs; features JSONB with 22 flags), `compliance_settings` (minimum wages, PLAWA, PTO, holiday pay, mileage, quality probation, re-clean workflow toggles per company), `feature_gates` (per-company feature overrides), `payroll_settings` (res_tech_pay_pct, commercial_hourly_rate, training_pay_rate, minimum_job_pay_hours, reclean_tech_rate, company_pay_floor, unavailable_reclassification_rate, quality_probation_* fields, mileage_rate — company_id UNIQUE).
- **Companies Table Extra Columns (raw ALTER TABLE, not in Drizzle):** `review_link`, `arrival_alert_window_minutes`, `addon_time_method`, `addon_minimum_minutes`, `addon_pct_of_base`, `tier_id` (FK subscription_tiers), `subscription_status`, `trial_ends_at`, `stripe_subscription_id`, `stripe_customer_id`, `pricing_model`, `early_tenant`.
